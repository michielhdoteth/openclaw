...
type ValidationPluginMetadataSnapshotLoader = {
  ...
  materialize: (config: OpenClawConfig) => ReturnType<typeof materializeRuntimeConfig>;
  getMetadata: () => ConfigPluginMetadata | undefined;
};
...
export function createConfigIoContext(options: ConfigIoFactoryOptions = {}): ConfigIoContext {
  ...
  function createValidationPluginMetadataSnapshotLoader(params: {
    ...
    const loader: ValidationPluginMetadataSnapshotLoader = {
      load: (config) => {
        metadata ??= resolveConfigWidePluginManifestRegistry({
          config,
          env: params.env,
          allowCurrent: params.allowCurrentPluginMetadata,
          pluginIdScope: resolvePluginIdScope(config),
        });
        return { manifestRegistry: metadata };
      },
      getMetadata: () => metadata,
      materialize: (config) =>
        materializeRuntimeConfig(config, {
          manifestRegistry:
            metadata ?? (options.pluginValidation === "core-only" ? { plugins: [] } : undefined),
