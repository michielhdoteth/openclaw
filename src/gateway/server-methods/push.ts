...
export const pushHandlers: GatewayRequestHandlers = {
  ...
  "push.web.preferences.get": async ({ params, respond, client }) => {
    if (!validateWebPushPreferencesGetParams(params)) {
      respondInvalidParams({
        respond,
        method: "push.web.preferences.get",
        validator: validateWebPushPreferencesGetParams,
      });
      return;
    }
    const deviceId = normalizeOptionalString(client?.connect.device?.id);
  ...
  "push.web.preferences.set": async ({ params, respond, client, context }) => {
    if (!validateWebPushPreferencesSetParams(params)) {
      respondInvalidParams({
        respond,
        method: "push.web.preferences.set",
        validator: validateWebPushPreferencesSetParams,
      });
      return;
    }
    const deviceId = normalizeOptionalString(client?.connect.device?.id);
    ...
    if (params.scope === "user") {
      ...
      const connIds = clientContext.getClientConnIds?.((connectedClient) => {
