      ...
      await withEnvAsync(
        {
          ...
          // Exercise product defaults consistently at both config reads and every CLI stage.
          VITEST: undefined,
        },
        async () => {
          ...
          if (kind.includes("provider")) {
            ...
            if (kind === "missing-direct-provider") {
              ...
              expect(read.pluginMetadata?.plugins.map((entry) => entry.id)).toContain(
                "defaults-owner",
              );
            }
          }
          const params = { cfg: read.config, session, primaryCommand: "prepared" };
          ...
          const scopes = discover.mock.calls.map(
            ([discoveryParams]) => discoveryParams?.workspaceDir,
          );
  ...
  it("retains skipped-validation metadata but rejects an invalid plugin at the fresh registration read", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(read.configDiagnostics).toBeNull();
        expect(read.pluginMetadata?.plugins.map(({ id }) => id)).toContain(plugin.id);
        ...
        expect(discover).toHaveBeenCalledTimes(initialDiscoveries);
        expect(getCurrentPluginMetadataSnapshot()).toBeUndefined();
        const program = new Command();
        expect(
          await registerPluginCliCommandsFromValidatedConfig(program, undefined, undefined, {
