...
Start an isolated chat from a Git-backed folder with a worktree session: on the Control UI's New session page, use the **Place** picker to choose a Gateway source folder, then select **Worktree** (with an optional base branch and worktree name). Choosing a paired device or cloud profile forces this managed-worktree path from the selected Gateway source; remote placement never browses or binds a node working directory. When the name is omitted, OpenClaw uses an explicit session label or an existing sidebar title, otherwise a crustacean-themed name. Worktree preparation never waits for title inference or uses the opening prompt as a branch name; the sidebar summary is generated alongside the admitted first turn. The choice appears for a Git-backed folder or a selected remote Git repository; ordinary folders can run directly only on the Gateway and show no Git isolation control. iOS exposes the same choice from Chat actions, and Android exposes it beside New Chat, when the active agent workspace is Git-backed.
...
`sessions.create` also accepts `worktreeBaseRef` and `worktreeName` alongside `worktree: true` to pick the base ref and the worktree name (the branch becomes `openclaw/<name>`); both stay at `operator.write`. If `worktreeName` is omitted, an available session label or sidebar title supplies the readable branch name, with a crustacean-themed fallback. For a new session with an initial message or task, creation returns the admitted session and run before worktree preparation finishes. The chat shows the submitted message and checkout and setup progress; failures remain visible and retryable in the same session. The agent starts only after the finalized worktree is bound. Creation without an initial turn still waits for preparation and returns the worktree in the create result. The bound checkout is persisted on the session row as `worktree: { id, branch, repoRoot }`, so session lists can show its checkout and branch. When session deletion cannot finish that cleanup, `worktreePreserved` identifies the active worktree record that needs attention and reports one bounded reason: owner mismatch, active use or competing cleanup, a foreign Git lock, snapshot failure, or another cleanup failure. These reasons describe cleanup and ownership, not whether the checkout is dirty or has unpushed commits.

### Publish a session pull request

For new automatically named session worktrees, `sessions.github.publish` uses the current sidebar label or summary for the GitHub branch, with a short worktree-specific suffix. If no usable ASCII slug is available, it keeps the neutral worktree branch. The local branch and checkout path never change.

Use the Gateway publication action or the `github_publish` agent tool for this naming flow. Direct Git or GitHub CLI publication keeps the branch you explicitly push.

The Gateway records the destination before pushing and reuses it after restarts and later title changes. Explicit worktree names, older sessions, and existing publication destinations stay unchanged. GitHub branch collisions fail visibly without overwriting unrelated work.

This publication contract uses shared-state schema 14. Older Gateways cannot open the upgraded database; see [database compatibility and downgrade recovery](/reference/database-schemas).
